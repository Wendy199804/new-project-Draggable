<template>
    <div class="risk-and-return4">
        <moduletitle title="情景分析">
            ...
            <div class="timeframe">
                <span>时段</span>
                <vDateRangePicker ref="range" type="year" class="mr" use="manager" @change="getChartData" />
            </div>
        </moduletitle>
        <div class="content">
            <div class="relative">
            ...
            <div>
                <div>
                    <el-table :data="tableData" v-loading="isLoading" height="282">
...
export default {
    ...
    methods: {
        ...
        handleTableData() {
            let emptyNum = Math.max(0, 6 - this.totalSize),
                emptyRows = new Array(emptyNum).fill({isEmpty: true})
            this.tableData = this.allTableData.concat(emptyRows)
        },
        handleChartData() {
            try {
        ...
        handleExcel() {
            let table_data = JSON.parse(JSON.stringify(this.allTableData)).map(item => {
